...
**IMPORTANT:** Never prefix individual dbt commands with `cd jaffle_beans &&`. The working directory persists between shell calls, so a second `cd jaffle_beans` from inside `jaffle_beans/` will fail. This is especially problematic with parallel shell calls.

Alternatively, use the Taskfile shortcuts from the project root:
...
**Skill selection rules — check in this order:**

1. **dbt:troubleshooting-dbt-job-errors** — use when a dbt job/build/run is **failing**, **broken**, or producing **errors**. Trigger phrases: "job is failing", "build failed", "debug errors", "tests are failing", "help me fix", "what went wrong". This takes priority over `using-dbt-for-analytics-engineering` for any failure/error investigation.
